...
public class DepartmentService {
	...
	public Mono<DeptEmpResponse> addDeptWithEmployees(DeptEmpRequest obj) {
		// TODO Auto-generated method stub
		// 1. Save department (blocking – acceptable in MVC apps)
		Department dept =deptRepo.save(obj.getDept());
		Long did = dept.getId();
		
		// 2. Set deptId for employees
		obj.getEmployees()
			.forEach(emp->emp.setDeptId(did));
			
		// 3. RETURN the WebClient reactive chain
		return webClient.post()
				.uri("/emp/addall")
				.header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
				.bodyValue(obj.getEmployees())
				.retrieve()
				.bodyToFlux(Employee.class)
				.collectList()
				.map(empList->new DeptEmpResponse(dept, empList));
